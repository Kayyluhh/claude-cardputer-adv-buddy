...
static void _applyJson(const char* line, TamaState* out) {
  ...
  // adjusted epoch yields local components, which we push into the
  // ESP32 internal RTC via Platform::setRtcLocal. Cardputer-Adv has
  // no battery-backed RTC chip, so the time persists only this boot.
  JsonArray t = doc["time"];
  if (!t.isNull() && t.size() == 2) {
    time_t local = (time_t)t[0].as<uint32_t>() + (int32_t)t[1];
    struct tm lt; gmtime_r(&local, &lt);
    Platform::setRtcLocal(lt);
    extern uint32_t _clkLastRead;
    _clkLastRead = 0;   // force re-read by the next clock-refresh tick
    _rtcValid = true;
    _lastLiveMs = millis();
    return;
  }
